...
import (
	...
	extnetworkpolicyscheme "github.com/ovn-org/ovn-kubernetes/go-controller/pkg/crd/extnetworkpolicy/v1alpha1/apis/clientset/versioned/scheme"
	extnetworkpolicyinformerfactory "github.com/ovn-org/ovn-kubernetes/go-controller/pkg/crd/extnetworkpolicy/v1alpha1/apis/informers/externalversions"
...
type WatchFactory struct {
	...
	enpFactory   extnetworkpolicyinformerfactory.SharedInformerFactory
	enpClientset extnetworkpolicyclientset.Interface
	crdFactory   apiextensionsinformerfactory.SharedInformerFactory
	informers    map[reflect.Type]*informer

	stopChan                 chan struct{}
	egressFirewallStopChan   chan struct{}
	extNetworkPolicyStopChan chan struct{}
}
...
var (
	...
	extNetworkPolicyType reflect.Type = reflect.TypeOf(&extnetworkpolicyapi.ExtNetworkPolicy{})
)
...
func NewMasterWatchFactory(ovnClientset *util.OVNClientset) (*WatchFactory, error) {
	...
	wf := &WatchFactory{
		...
		enpClientset: ovnClientset.ExtNetworkPolicyClient,
...
func (wf *WatchFactory) InitializeExtNetworkPolicyWatchFactory() error {
	err := extnetworkpolicyapi.AddToScheme(extnetworkpolicyscheme.Scheme)
	if err != nil {
		return err
	}
	wf.enpFactory = extnetworkpolicyinformerfactory.NewSharedInformerFactory(wf.enpClientset, resyncInterval)
	wf.informers[extNetworkPolicyType], err = newInformer(extNetworkPolicyType, wf.enpFactory.K8s().V1alpha1().ExtNetworkPolicies().Informer())
	if err != nil {
		return err
	}
	wf.extNetworkPolicyStopChan = make(chan struct{})
	wf.enpFactory.Start(wf.extNetworkPolicyStopChan)
	for oType, synced := range wf.enpFactory.WaitForCacheSync(wf.extNetworkPolicyStopChan) {
		if !synced {
			return fmt.Errorf("error in syncing cache for %v informer", oType)
		}
	}
...
func (wf *WatchFactory) ShutdownExtNetworkPolicyWatchFactory() {
	close(wf.extNetworkPolicyStopChan)
	wf.informers[extNetworkPolicyType].shutdown()
}
...
func getObjectMeta(objType reflect.Type, obj interface{}) (*metav1.ObjectMeta, error) {
	...
	case extNetworkPolicyType:
		if extNetworkPolicy, ok := obj.(*extnetworkpolicyapi.ExtNetworkPolicy); ok {
			return &extNetworkPolicy.ObjectMeta, nil
		}
	}
...
// AddExtNetworkPolicyHandler adds a handler function that will be executed on ExtNetworkPolicy object changes
func (wf *WatchFactory) AddExtNetworkPolicyHandler(handlerFuncs cache.ResourceEventHandler, processExisting func([]interface{})) *Handler {
	return wf.addHandler(extNetworkPolicyType, "", nil, handlerFuncs, processExisting)
}

// RemoveExtNetworkPolicyHandler removes an ExtNetworkPolicy object event handler function
func (wf *WatchFactory) RemoveExtNetworkPolicyHandler(handler *Handler) {
	wf.removeHandler(extNetworkPolicyType, handler)
}
